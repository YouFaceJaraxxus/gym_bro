...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST" && !id) {
    ...
    await db.updateTable("users").set({ role: "employee" }).where("id", "=", user_id).execute();
    return json(row, 201);
  }
  ...
  if (req.method === "GET" && !id) {
    ...
    let query = db.selectFrom("employee").selectAll();
  ...
  // ── GET /employees/:id ────────────────────────────────────────────────────────
  if (req.method === "GET" && id) {
    const row = await db.selectFrom("employee").selectAll().where("id", "=", id).executeTakeFirst();
